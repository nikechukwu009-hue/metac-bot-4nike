...
# NOTE: DataOrganizer is used inside forecasting_tools parsing paths.
# We patch it globally so int bounds get coerced to float regardless of call path.
...
# -----------------------------
# OpenRouter/LiteLLM model IDs (IMPORTANT)
# -----------------------------
# With LiteLLM, OpenRouter models should be "openrouter/<openrouter_model_id>".
# If you want OpenRouter’s router called "openrouter/free", you must pass:
#   "openrouter/openrouter/free"
#
# You can also use a specific free model, e.g.:
...
def _coerce_int_bounds_to_float(obj: Any) -> Any:
    """
    Recursively convert int bounds to float bounds so forecasting_tools assertions pass.
    """
...
# -----------------------------
# GLOBAL PATCH: DataOrganizer parsing
# -----------------------------
# This is the most reliable place to fix the int->float bound assertion, because
# all question parsing ends up going through DataOrganizer.get_question_from_post_json.
...
# Make sure it's treated like the original (usually a @staticmethod).
...
class PatchedMetaculusClient(MetaculusClient):
    """
    Compatibility layer: some templates call one of these names.
    Your traceback shows your code expects one of:
      - get_questions_from_tournament
      - get_questions_in_tournament
      - get_tournament_questions
    We implement all three and delegate to whichever real method exists upstream.
    """

    async def get_questions_from_tournament(self, tournament_id_or_slug: Union[int, str]):
        return await self._get_open_tournament_questions(tournament_id_or_slug)

    async def get_questions_in_tournament(self, tournament_id_or_slug: Union[int, str]):
        return await self._get_open_tournament_questions(tournament_id_or_slug)

    async def get_tournament_questions(self, tournament_id_or_slug: Union[int, str]):
        return await self._get_open_tournament_questions(tournament_id_or_slug)

    async def _get_open_tournament_questions(self, tournament_id_or_slug: Union[int, str]):
        """
        Try common upstream method names, across forecasting_tools versions.
        """
        # Newer naming (as documented): get_all_open_questions_from_tournament
        if hasattr(self, "get_all_open_questions_from_tournament"):
            fn = getattr(self, "get_all_open_questions_from_tournament")
            out = fn(tournament_id_or_slug)
            return await out if asyncio.iscoroutine(out) else out

        # Older naming patterns
        for name in ("get_open_questions_from_tournament", "get_questions_from_tournament"):
        ...
        raise AttributeError(
            "No tournament question retrieval method found on MetaculusClient. "
...
class LinkupExaSpringBot2026(ForecastBot):
    ...
    def __init__(self, *args, **kwargs):
        kwargs["llms"] = {
            "default": GeneralLlm(model=OPENROUTER_DEFAULT_MODEL, temperature=0.2, timeout=60, allowed_tries=2),
            "parser": GeneralLlm(model=OPENROUTER_PARSER_MODEL, temperature=0.0, timeout=60, allowed_tries=2),
            "summarizer": GeneralLlm(model=OPENROUTER_DEFAULT_MODEL, temperature=0.2, timeout=60, allowed_tries=2),
            "researcher": GeneralLlm(model=OPENROUTER_DEFAULT_MODEL, temperature=0.2, timeout=60, allowed_tries=2),
        }
    ...
    def _maybe_extremize_binary(self, p: float, ev: EvidenceSummary) -> float:
        if ev.strength >= self._evidence_extremize_threshold and ev.direction in ("yes", "no"):
            target = self._extremize_target if ev.direction == "yes" else (1.0 - self._extremize_target)
            return _to_prob_decimal(target)
    ...
    async def _sample_median_multichoice(
        ...
        return ReasonedPrediction(prediction_value=final_list, reasoning="")
    ...
    async def _sample_median_numeric(
        ...
        parsing_instructions = clean_indents(
            """
            Return only the requested percentiles. Do not hallucinate missing values.
            """
        )
        ...
        return ReasonedPrediction(prediction_value=dist, reasoning="")
    ...
    async def _run_forecast_on_conditional(self, question: ConditionalQuestion, research: str) -> ReasonedPrediction[ConditionalPrediction]:
        # Conditional questions are "child given parent" (see your earlier question).
        ...
        return ReasonedPrediction(reasoning="", prediction_value=full_prediction)
...
async def main():
    ...
    if run_mode == "tournament":
        # one event loop, run sequentially (or make concurrent if desired)
        seasonal = await bot.forecast_on_tournament(client.CURRENT_AI_COMPETITION_ID, return_exceptions=True)
        minibench = await bot.forecast_on_tournament(client.CURRENT_MINIBENCH_ID, return_exceptions=True)
        market_pulse = await bot.forecast_on_tournament(MARKET_PULSE_TOURNAMENT_SLUG, return_exceptions=True)
        reports = seasonal + minibench + market_pulse
    elif run_mode == "metaculus_cup":
        bot.skip_previously_forecasted_questions = False
        reports = await bot.forecast_on_tournament(client.CURRENT_METACULUS_CUP_ID, return_exceptions=True)
    else:
        EXAMPLE_QUESTIONS = [
